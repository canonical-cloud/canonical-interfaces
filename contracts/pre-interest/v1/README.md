...
organization name, referral code, and free-form payloads never enter URLs,
Cloudflare headers, logs, metrics, traces, idempotency keys, or error bodies.
...
`PreInterestRegistrationRequest` uses tags 1 through 10 exactly as reviewed in
`pre_interest.proto`. Removed fields must be reserved rather than renumbered.
Optional string presence is significant for organization name, locale, and
referral code.
