// AUTOGENERATED by src/generate-quote-dart.mjs — do not edit by hand; edit schema/ and regenerate.

final class QuoteContactSelectionRequest {
  const QuoteContactSelectionRequest({required this.emailConfirmed, required this.phoneConfirmed});
  final bool emailConfirmed;
  final bool phoneConfirmed;
  factory QuoteContactSelectionRequest.fromJson(Map<String, Object?> json) => QuoteContactSelectionRequest(
    emailConfirmed: json['emailConfirmed'] as bool,
    phoneConfirmed: json['phoneConfirmed'] as bool,
  );
  Map<String, Object?> toJson() => {
    'emailConfirmed': emailConfirmed,
    'phoneConfirmed': phoneConfirmed,
  };
}

final class QuoteContactSelection {
  const QuoteContactSelection({required this.contactSelectionId, required this.emailMasked, required this.phoneMasked, required this.expiresAt});
  final String contactSelectionId;
  final String emailMasked;
  final String phoneMasked;
  final String expiresAt;
  factory QuoteContactSelection.fromJson(Map<String, Object?> json) => QuoteContactSelection(
    contactSelectionId: json['contactSelectionId'] as String,
    emailMasked: json['emailMasked'] as String,
    phoneMasked: json['phoneMasked'] as String,
    expiresAt: json['expiresAt'] as String,
  );
  Map<String, Object?> toJson() => {
    'contactSelectionId': contactSelectionId,
    'emailMasked': emailMasked,
    'phoneMasked': phoneMasked,
    'expiresAt': expiresAt,
  };
}

final class QuoteRequest {
  const QuoteRequest({required this.organizationName, required this.contactName, required this.contactSelectionId, this.website, required this.employeeCount, this.annualRevenueBand, required this.frameworks, required this.currentStage, required this.infrastructure, required this.dataSensitivity, this.targetDate, required this.hasSecurityProgram, required this.hasPolicies, required this.hasRiskAssessment, required this.hasIncidentResponsePlan, required this.hasVendorManagement, this.notes, this.contextKey, required this.answersVersion});
  final String organizationName;
  final String contactName;
  final String contactSelectionId;
  final String? website;
  final int employeeCount;
  final String? annualRevenueBand;
  final List<String> frameworks;
  final String currentStage;
  final List<String> infrastructure;
  final List<String> dataSensitivity;
  final String? targetDate;
  final bool hasSecurityProgram;
  final bool hasPolicies;
  final bool hasRiskAssessment;
  final bool hasIncidentResponsePlan;
  final bool hasVendorManagement;
  final String? notes;
  final String? contextKey;
  final int answersVersion;
  factory QuoteRequest.fromJson(Map<String, Object?> json) => QuoteRequest(
    organizationName: json['organizationName'] as String,
    contactName: json['contactName'] as String,
    contactSelectionId: json['contactSelectionId'] as String,
    website: json['website'] == null ? null : json['website'] as String,
    employeeCount: json['employeeCount'] as int,
    annualRevenueBand: json['annualRevenueBand'] == null ? null : json['annualRevenueBand'] as String,
    frameworks: (json['frameworks'] as List).cast<String>(),
    currentStage: json['currentStage'] as String,
    infrastructure: (json['infrastructure'] as List).cast<String>(),
    dataSensitivity: (json['dataSensitivity'] as List).cast<String>(),
    targetDate: json['targetDate'] == null ? null : json['targetDate'] as String,
    hasSecurityProgram: json['hasSecurityProgram'] as bool,
    hasPolicies: json['hasPolicies'] as bool,
    hasRiskAssessment: json['hasRiskAssessment'] as bool,
    hasIncidentResponsePlan: json['hasIncidentResponsePlan'] as bool,
    hasVendorManagement: json['hasVendorManagement'] as bool,
    notes: json['notes'] == null ? null : json['notes'] as String,
    contextKey: json['contextKey'] == null ? null : json['contextKey'] as String,
    answersVersion: json['answersVersion'] as int,
  );
  Map<String, Object?> toJson() => {
    'organizationName': organizationName,
    'contactName': contactName,
    'contactSelectionId': contactSelectionId,
    if (website != null) 'website': website!,
    'employeeCount': employeeCount,
    if (annualRevenueBand != null) 'annualRevenueBand': annualRevenueBand!,
    'frameworks': frameworks,
    'currentStage': currentStage,
    'infrastructure': infrastructure,
    'dataSensitivity': dataSensitivity,
    if (targetDate != null) 'targetDate': targetDate!,
    'hasSecurityProgram': hasSecurityProgram,
    'hasPolicies': hasPolicies,
    'hasRiskAssessment': hasRiskAssessment,
    'hasIncidentResponsePlan': hasIncidentResponsePlan,
    'hasVendorManagement': hasVendorManagement,
    if (notes != null) 'notes': notes!,
    if (contextKey != null) 'contextKey': contextKey!,
    'answersVersion': answersVersion,
  };
}

final class QuoteSubmissionResponse {
  const QuoteSubmissionResponse({required this.quoteId, required this.revision, required this.status, required this.streamUrl, required this.accessLinkExpiresAt, required this.createdAt});
  final String quoteId;
  final int revision;
  final String status;
  final String streamUrl;
  final String accessLinkExpiresAt;
  final String createdAt;
  factory QuoteSubmissionResponse.fromJson(Map<String, Object?> json) => QuoteSubmissionResponse(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    status: json['status'] as String,
    streamUrl: json['streamUrl'] as String,
    accessLinkExpiresAt: json['accessLinkExpiresAt'] as String,
    createdAt: json['createdAt'] as String,
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'status': status,
    'streamUrl': streamUrl,
    'accessLinkExpiresAt': accessLinkExpiresAt,
    'createdAt': createdAt,
  };
}

final class QuoteEstimate {
  const QuoteEstimate({required this.quoteId, required this.revision, required this.status, required this.currency, required this.lowerBoundCents, required this.upperBoundCents, required this.durationWeeksLow, required this.durationWeeksHigh, required this.confidence, required this.summary, required this.assumptions, required this.gaps, required this.nextSteps, required this.frameworks, required this.model, required this.contextVersion, required this.createdAt});
  final String quoteId;
  final int revision;
  final String status;
  final String currency;
  final int lowerBoundCents;
  final int upperBoundCents;
  final int durationWeeksLow;
  final int durationWeeksHigh;
  final String confidence;
  final String summary;
  final List<String> assumptions;
  final List<String> gaps;
  final List<String> nextSteps;
  final List<String> frameworks;
  final String model;
  final String contextVersion;
  final String createdAt;
  factory QuoteEstimate.fromJson(Map<String, Object?> json) => QuoteEstimate(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    status: json['status'] as String,
    currency: json['currency'] as String,
    lowerBoundCents: json['lowerBoundCents'] as int,
    upperBoundCents: json['upperBoundCents'] as int,
    durationWeeksLow: json['durationWeeksLow'] as int,
    durationWeeksHigh: json['durationWeeksHigh'] as int,
    confidence: json['confidence'] as String,
    summary: json['summary'] as String,
    assumptions: (json['assumptions'] as List).cast<String>(),
    gaps: (json['gaps'] as List).cast<String>(),
    nextSteps: (json['nextSteps'] as List).cast<String>(),
    frameworks: (json['frameworks'] as List).cast<String>(),
    model: json['model'] as String,
    contextVersion: json['contextVersion'] as String,
    createdAt: json['createdAt'] as String,
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'status': status,
    'currency': currency,
    'lowerBoundCents': lowerBoundCents,
    'upperBoundCents': upperBoundCents,
    'durationWeeksLow': durationWeeksLow,
    'durationWeeksHigh': durationWeeksHigh,
    'confidence': confidence,
    'summary': summary,
    'assumptions': assumptions,
    'gaps': gaps,
    'nextSteps': nextSteps,
    'frameworks': frameworks,
    'model': model,
    'contextVersion': contextVersion,
    'createdAt': createdAt,
  };
}

final class QuoteStatusEvent {
  const QuoteStatusEvent({required this.quoteId, required this.revision, required this.sequence, required this.stage, required this.message, required this.terminal, this.estimate, required this.occurredAt, this.problem});
  final String quoteId;
  final int revision;
  final String sequence;
  final String stage;
  final String message;
  final bool terminal;
  final QuoteEstimate? estimate;
  final String occurredAt;
  final QuoteProblem? problem;
  factory QuoteStatusEvent.fromJson(Map<String, Object?> json) => QuoteStatusEvent(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    sequence: json['sequence'] as String,
    stage: json['stage'] as String,
    message: json['message'] as String,
    terminal: json['terminal'] as bool,
    estimate: json['estimate'] == null ? null : QuoteEstimate.fromJson((json['estimate'] as Map).cast<String, Object?>()),
    occurredAt: json['occurredAt'] as String,
    problem: json['problem'] == null ? null : QuoteProblem.fromJson((json['problem'] as Map).cast<String, Object?>()),
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'sequence': sequence,
    'stage': stage,
    'message': message,
    'terminal': terminal,
    if (estimate != null) 'estimate': estimate!.toJson(),
    'occurredAt': occurredAt,
    if (problem != null) 'problem': problem!.toJson(),
  };
}

final class QuoteProblem {
  const QuoteProblem({required this.code, required this.message, required this.requestId});
  final String code;
  final String message;
  final String requestId;
  factory QuoteProblem.fromJson(Map<String, Object?> json) => QuoteProblem(
    code: json['code'] as String,
    message: json['message'] as String,
    requestId: json['requestId'] as String,
  );
  Map<String, Object?> toJson() => {
    'code': code,
    'message': message,
    'requestId': requestId,
  };
}

final class QuoteSummary {
  const QuoteSummary({required this.quoteId, required this.revision, required this.status, required this.organizationName, required this.frameworks, required this.createdAt, required this.updatedAt, this.estimate});
  final String quoteId;
  final int revision;
  final String status;
  final String organizationName;
  final List<String> frameworks;
  final String createdAt;
  final String updatedAt;
  final QuoteEstimate? estimate;
  factory QuoteSummary.fromJson(Map<String, Object?> json) => QuoteSummary(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    status: json['status'] as String,
    organizationName: json['organizationName'] as String,
    frameworks: (json['frameworks'] as List).cast<String>(),
    createdAt: json['createdAt'] as String,
    updatedAt: json['updatedAt'] as String,
    estimate: json['estimate'] == null ? null : QuoteEstimate.fromJson((json['estimate'] as Map).cast<String, Object?>()),
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'status': status,
    'organizationName': organizationName,
    'frameworks': frameworks,
    'createdAt': createdAt,
    'updatedAt': updatedAt,
    if (estimate != null) 'estimate': estimate!.toJson(),
  };
}

final class QuoteDetail {
  const QuoteDetail({required this.quoteId, required this.revision, required this.status, required this.request, required this.eventsUrl, required this.accessLinkExpiresAt, required this.createdAt, required this.updatedAt, this.estimate, this.problem});
  final String quoteId;
  final int revision;
  final String status;
  final QuoteRequest request;
  final String eventsUrl;
  final String accessLinkExpiresAt;
  final String createdAt;
  final String updatedAt;
  final QuoteEstimate? estimate;
  final QuoteProblem? problem;
  factory QuoteDetail.fromJson(Map<String, Object?> json) => QuoteDetail(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    status: json['status'] as String,
    request: QuoteRequest.fromJson((json['request'] as Map).cast<String, Object?>()),
    eventsUrl: json['eventsUrl'] as String,
    accessLinkExpiresAt: json['accessLinkExpiresAt'] as String,
    createdAt: json['createdAt'] as String,
    updatedAt: json['updatedAt'] as String,
    estimate: json['estimate'] == null ? null : QuoteEstimate.fromJson((json['estimate'] as Map).cast<String, Object?>()),
    problem: json['problem'] == null ? null : QuoteProblem.fromJson((json['problem'] as Map).cast<String, Object?>()),
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'status': status,
    'request': request.toJson(),
    'eventsUrl': eventsUrl,
    'accessLinkExpiresAt': accessLinkExpiresAt,
    'createdAt': createdAt,
    'updatedAt': updatedAt,
    if (estimate != null) 'estimate': estimate!.toJson(),
    if (problem != null) 'problem': problem!.toJson(),
  };
}

final class QuoteListQuery {
  const QuoteListQuery({this.cursor, this.limit});
  final String? cursor;
  final int? limit;
  factory QuoteListQuery.fromJson(Map<String, Object?> json) => QuoteListQuery(
    cursor: json['cursor'] == null ? null : json['cursor'] as String,
    limit: json['limit'] == null ? null : json['limit'] as int,
  );
  Map<String, Object?> toJson() => {
    if (cursor != null) 'cursor': cursor!,
    if (limit != null) 'limit': limit!,
  };
}

final class QuoteListResponse {
  const QuoteListResponse({required this.quotes, this.nextCursor});
  final List<QuoteSummary> quotes;
  final String? nextCursor;
  factory QuoteListResponse.fromJson(Map<String, Object?> json) => QuoteListResponse(
    quotes: (json['quotes'] as List).map((value) => QuoteSummary.fromJson((value as Map).cast<String, Object?>())).toList(growable: false),
    nextCursor: json['nextCursor'] == null ? null : json['nextCursor'] as String,
  );
  Map<String, Object?> toJson() => {
    'quotes': quotes.map((value) => value.toJson()).toList(growable: false),
    if (nextCursor != null) 'nextCursor': nextCursor!,
  };
}

final class QuoteRetryResponse {
  const QuoteRetryResponse({required this.quoteId, required this.revision, required this.status, required this.streamUrl, required this.updatedAt});
  final String quoteId;
  final int revision;
  final String status;
  final String streamUrl;
  final String updatedAt;
  factory QuoteRetryResponse.fromJson(Map<String, Object?> json) => QuoteRetryResponse(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    status: json['status'] as String,
    streamUrl: json['streamUrl'] as String,
    updatedAt: json['updatedAt'] as String,
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'status': status,
    'streamUrl': streamUrl,
    'updatedAt': updatedAt,
  };
}

final class QuoteResubmissionResponse {
  const QuoteResubmissionResponse({required this.quoteId, required this.revision, required this.status, required this.streamUrl, required this.accessLinkExpiresAt, required this.updatedAt});
  final String quoteId;
  final int revision;
  final String status;
  final String streamUrl;
  final String accessLinkExpiresAt;
  final String updatedAt;
  factory QuoteResubmissionResponse.fromJson(Map<String, Object?> json) => QuoteResubmissionResponse(
    quoteId: json['quoteId'] as String,
    revision: json['revision'] as int,
    status: json['status'] as String,
    streamUrl: json['streamUrl'] as String,
    accessLinkExpiresAt: json['accessLinkExpiresAt'] as String,
    updatedAt: json['updatedAt'] as String,
  );
  Map<String, Object?> toJson() => {
    'quoteId': quoteId,
    'revision': revision,
    'status': status,
    'streamUrl': streamUrl,
    'accessLinkExpiresAt': accessLinkExpiresAt,
    'updatedAt': updatedAt,
  };
}
